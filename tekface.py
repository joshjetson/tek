...
def main():
    ...
    ap.add_argument("--model", default="anat",
                    choices=("anat", "fdl", "mannequin", "face"),
    ...
    if a.model == "fdl":
        verts, edges, normals = tekfdl.build()
        print("building mouth pose table...", flush=True)
        tekfdl.build_pose_table(len(verts))
        dist = a.dist if a.dist != 3.15 else 16.0
        fov = a.fov if a.fov else 11.4
    elif a.model == "anat":
    ...
    talking = a.model in ("anat", "fdl")

    def pose(rx, ry, t=None):
        v, e_, n_ = verts, edges, normals
        if talking and t is not None and a.model == "fdl":
            op, rnd = tekfdl.speech_params(t)
            mv, me, mn = tekfdl.mouth_pose(op, rnd)
            if len(mv):
                v = np.concatenate([verts, mv])
                n_ = np.concatenate([normals, mn])
                e_ = np.concatenate([edges, me])
        elif talking and t is not None:
