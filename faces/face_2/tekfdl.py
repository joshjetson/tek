...
def _march(mask, gx, gy):
    ...
    for c in cs:
        if len(c) < 12:
            continue
        c = cv2.approxPolyDP(c, 1.15, True)       # simplify
        if len(c) < 6:
            continue
        p = c[:, 0, :].astype(float)
...
def build(res=360, lips=False, z_top=0.95, z_bot=-0.25, z_step=0.05, depth=1.05,
          back=True):
    """Returns (verts, edges, normals)."""
    gx = np.linspace(-1.02, 1.02, res)
    gy = np.linspace(1.18, -1.48, res)      # extended to cover the neck
    ...
    for lev in levels:
        for poly in _march(Zm >= lev, gx, gy):
            # split the loop wherever it enters the mouth box, so the animated
            # mouth can supply that region without double-drawing
            inside = ((poly[:, 0] > bx0) & (poly[:, 0] < bx1)
                      & (poly[:, 1] > by0) & (poly[:, 1] < by1))
            if not inside.any():
                emit(poly, float(lev))
                continue
            run = []
