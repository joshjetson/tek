...
def field_brows(X, Y, c):
    ...
    pts = [(x, y + 0.075 * raise_ - 0.045 * furrow * (1.0 - abs(x) / 0.34))
           for (x, y) in F.BROW_POLY]
    z = F._ridge(X, Y, pts, 0.105 + 0.030 * furrow, 0.085, mirror=True)
...
def field_eyes(X, Y, c):
    ...
    for sgn in (-1.0, 1.0):
        ...
        cy = F.EYE["cy"]
        # socket
        z = z + F.EYE["a"] * np.exp(-((X - cx) ** 2) / F.EYE["rx2"]
                                    - ((Y - cy) ** 2) / F.EYE["ry2"])
...
def field_mouth(X, Y, c):
    ...
    z = F.lip_field(X, Y, c["mouth_open"], c["mouth_round"])
    sm = c["smile"]
    if abs(sm) > 1e-3:
...
class Region:
    ...
    def __init__(self, name, spec, res=None, depth=1.05, z_step=0.05):
        ...
        self.base = F.zfield(X, Y, lips=False) - self.fn(X, Y, DEFAULTS)
        self.mask = np.abs(X) <= F.sil_w(Y)
        self.cache = _Cache()
    ...
    def _contour(self, controls):
        ...
        for lev in np.arange(hi, lo - 1e-9, -self.z_step):
            ...
            for poly in F._march(Zm >= float(lev), self.gx, self.gy,
...
class Face:
    ...
    def __init__(self, verbose=False):
        ...
        self.static = F.build(lips=False)
